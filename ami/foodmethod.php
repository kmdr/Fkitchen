...
$msql = "SELECT method.order, action.name as action, food.name as food FROM method INNER JOIN action ON method.aid = action.id INNER JOIN food ON method.fid = food.id WHERE "; 

if(isset($_GET['id'])) {
  $msql .= "method.rid=".mysqli_real_escape_string($link, $_GET['id']);
}
